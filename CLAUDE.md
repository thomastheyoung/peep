...
- `themes/*.css` — 22 CSS theme files. Each opens with a `/*! @name / @description / @author */` frontmatter block, which the extractor reads so `registry.ts` need not hand-author names. Injected at runtime by setting `.textContent` on a `<style id="md-theme">` node — deliberately NOT `{@html}`, which is an XSS sink for imported themes (a theme containing the style-closing sequence breaks out into live markup)
